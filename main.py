...
def try_agent():
    ...
    agent = MachineAgent(network, 2, 4, problem_type='mountaincar_cont')
...
def train_network(name, save=True):
    trained_network = genetic_algorithm(population_size=250, generations=5, in_size=2, out_size=1, layers=[
                                        5, 5], mutation_rate=0.01, mutation_amount=0.1, crossover_rate=0.5, problem_type='mountaincar_cont')
    if save:
